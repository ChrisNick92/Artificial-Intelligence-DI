...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    PA_1, PA_0 = PropSymbolExpr('PacmanAlive', time=1), PropSymbolExpr('PacmanAlive', time=0)
    PB_0, PK_0 = PropSymbolExpr('PacmanBorn', time=0), PropSymbolExpr('PacmanKilled', time=0)
    
    p1 = PA_1 % disjoin([conjoin([PA_0, ~PK_0]), conjoin([~PA_0, PB_0])])
    p2 = ~conjoin([PA_0, PB_0])
    p3 = PB_0
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return conjoin([disjoin(~expr1, ~expr2) for expr1, expr2 in itertools.combinations(literals, 2)])
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            expr = pacmanSuccessorAxiomSingle(x, y, t+1, walls_grid)
            if expr:
                KB.append(expr)
        # KB.append([pacmanSuccessorAxiomSingle(x, y, t+1, walls_grid) for x, y in non_wall_coords])
    # util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        print(f"Time step: {t}")
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_wall_coords]))
        goal_t = conjoin([~PropSymbolExpr(food_str, x, y, time=t) for x, y in food])
        model = findModel(conjoin([conjoin(KB), goal_t]))
        if model:
            return extractActionSequence(model, actions)
        KB.append(exactlyOne([PropSymbolExpr(action, time=t) for action in DIRECTIONS]))
        KB += [conjoin([PropSymbolExpr(food_str, x, y, time=t), ~PropSymbolExpr(pacman_str, x, y, time=t)]) >> PropSymbolExpr(food_str, x, y, time=t+1) for x, y in food]
        for x, y in non_wall_coords:
            expr = pacmanSuccessorAxiomSingle(x, y, t+1, walls)
            if expr:
                KB.append(expr)
    # util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # util.raiseNotDefined()
    KB += [PropSymbolExpr(wall_str, x, y) for x, y in walls_list]
    KB += [~PropSymbolExpr(wall_str, x, y) for x, y in list(filter(lambda x: x not in walls_list, all_coords))]
    for t in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(t=t, all_coords=all_coords, non_outer_wall_coords=non_outer_wall_coords, sensorModel=sensorAxioms,
...
def mapping(problem, agent) -> Generator:
    ...
    if (pac_x_0, pac_y_0) in non_outer_wall_coords:
    ...
    for t in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(t=t, all_coords=all_coords, non_outer_wall_coords=non_outer_wall_coords, sensorModel=sensorAxioms,
                            walls_grid=known_map, successorAxioms=allLegalSuccessorAxioms))
        KB.append(PropSymbolExpr(agent.actions[t], time=t))
        KB.append(fourBitPerceptRules(t, agent.getPercepts()))
        for x, y in non_outer_wall_coords:
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        possible_locations = []
        KB.append(pacphysicsAxioms(t=t, all_coords=all_coords, non_outer_wall_coords=non_outer_wall_coords, sensorModel=SLAMSensorAxioms,
                    walls_grid=known_map, successorAxioms=SLAMSuccessorAxioms))
        KB.append(PropSymbolExpr(agent.actions[t], time=t))
